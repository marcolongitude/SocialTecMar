<?php 	
	...
	function contaDados ($inicio,$fim){
		...
		$sql = "SELECT * FROM Plano WHERE dataInicialPlano BETWEEN ('$inicio') and ('$fim')";
		$result = mysqli_query($conexao,$sql) or die ('ERRO DE PESQUISA NA TABELA Visita - ARQ graficoVisitas VARIAVEL result');
		$total = 0;
		while ($row = mysqli_fetch_array($result)){
			$decimal = explode(',',$row['valorPlano']);
			$valorI = str_replace('.','',$decimal[0]);
			(double)$total += (double)$valorI;
			
		}
		
		//$totalT = str_replace('.','',$total);
		//$totalT = str_replace(',','.',$total);		
